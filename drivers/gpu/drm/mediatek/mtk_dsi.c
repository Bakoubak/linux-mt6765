...
static irqreturn_t mtk_dsi_irq(int irq, void *dev_id)
{
         ...
         u32 status, tmp, raw_status;
         u32 flag = LPRX_RD_RDY_INT_FLAG | CMD_DONE_INT_FLAG | VM_DONE_INT_FLAG;

         raw_status = readl(dsi->regs + DSI_INTSTA);
         if (raw_status != 0) {
                 printk(KERN_INFO "[HIMAX_PRINTK] IRQ DSI déclenchée ! DSI_INTSTA brut = 0x%08x\n", raw_status);
         }

         status = raw_status & flag;
         ...
         if (status) {
                 ...
                 printk(KERN_WARNING "[HIMAX_PRINTK] Interruption ignorée car hors-masque (flag=0x%08x)\n", flag);
                 writel(raw_status, dsi->regs + DSI_INTSTA); 
         }
